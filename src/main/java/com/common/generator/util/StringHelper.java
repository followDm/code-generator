...
import com.common.generator.provider.db.table.model.Column.EnumMetaDada;
...
//	    Pattern p = Pattern.compile("<is\\w+\\s+[\\w\\s='\"]+>\\s*order\\s+by.*?</\\w+>");
	...
	/**
	 * 把存储过程名转换成java类名，驼峰规则首字母大写
	 * (如：test_in_out_array 转换为 TestInOutArray)
	 * @param proName
	 * @return
	 */
	public static String convertToClassName(String proName){
		String[] names = proName.split("_");
		StringBuffer sb = new StringBuffer();
	...
	/**
	 * oracle存储过程中的参数名称对应java DTO属性名称及ibatis配置中的名称
	 * (驼峰规则,如p_atest_sid 生成为patestSid,这里注意的是第1根下划线后面的值是不大写的)
	 * @param parameterName
	 * @return
	 */
	public static String convertToParameterJavaName(String parameterName){
		String[] names = parameterName.split("_");
		StringBuffer sb = new StringBuffer();
		for(int i=0;i<names.length;i++){
			if(i==0 || i==1){
				sb.append(names[i]);
	...
	/**
	 * 把解析器解析出来的数据类型转换成标准形式
	 * 如：number(16)转换成 number
	 * @param dataType
	 * @return
	 */
