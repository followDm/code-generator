...
//		if(outRootDir == null) throw new IllegalStateException("'outRootDir' property must be not null.");
	...
	/**
	 * 根据模板文件路径,生成器模板可以引用的变量,文件路径可以引用的变量,是否删除标志，
	 * 来生成或者删除文件
		...
		for(int i = 0; i < srcFiles.size(); i++) {
			...
			try {
			    if(isDelete){
			    	...
			    	if(name.endsWith("Po")){
			    		for(int j=0;j<3;j++){
			    			filePathModel.put("className", (String)filePathModel.get("className")+j);
	...
	private class TemplateProcessor {
		...
		private void executeGenerate(File templateRootDir,Map templateModel, Map filePathModel ,File srcFile) throws SQLException, IOException,TemplateException {
			...
			//后缀为include,或者文件夹则直接跳过
			if(GeneratorHelper.isIgnoreTemplateProcess(srcFile, templateFile,includes,excludes)) {
				return;
			}
			templateModel.putAll(filePathModel);
			
			//自动拷贝二进制文件至输出目录(如模板目录下的 zip,rar,doc文件将会自动拷贝至输出目录,不会破坏文件格式)
			if((isCopyBinaryFile && FileHelper.isBinaryFile(srcFile)) || "ProjectConfig.java".equals(srcFile.getName())) {
				//处理文件路径的变量变成输出路径
            ...
            try {
            	/****
        		 * 得到输出文件的路径名，
        		 * 如：java\com/picc/product/basic\po\DelAtestStudent2Po.java
        		 */
                String outputFilepath = proceeForOutputFilepath(filePathModel,templateFile);    
                //初始化控制代码生成的属性
	...
	static class GeneratorHelper {
		public static boolean isIgnoreTemplateProcess(File srcFile,String templateFile,String includes,String excludes) {
			...
			//modified by mark 2015年3月16日
			//若为系统配置文件，则直接跳过
			if(srcFile.getName().startsWith("ProjectConfig")){
				GLogger.println("[skip]\t\t filename is ProjectConfig.java:"+templateFile);
				return true;
			}
			/*
			if(srcFile.getName().toLowerCase().endsWith("context.xml")){
				GLogger.println("[skip]\t\t filename is context.xml:"+templateFile);
				return true;
			}
			*/
...
//			GLogger.info("[set Freemarker.autoIncludes]"+availableAutoInclude+" for templateName:"+templateName);
	...
	public static class GeneratorModel implements java.io.Serializable{		    
		public Map filePathModel;//filePathModel 文件路径可以引用的变量
		public Map templateModel;//templateModel 生成器模板可以引用的变量
